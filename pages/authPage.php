...
$new_user = $_REQUEST['login'];
//ищем такого пользователя
$q = mysql_query("select count(*) as kol from profile where email = '" . $new_user . "'");
while ($r = mysql_fetch_assoc($q)) {
  $kk = $r['kol'];
}
if ($kk > 0) {
  //Выбираем данные пользователя и показываем их
  $q = mysql_query("select * from profile where email = '" . $new_user . "'");
  while ($r = mysql_fetch_assoc($q)) {
    $user_id = $r['id'];
    $name = $r['name'];
    $type = $r['type'];
  ...
  if ($type == 'Физическое лицо') {
    ...
    echo'<b>Email: </b> ' . $new_user . ' 
												 <input type="text" id="enew_user" value="' . $new_user . '" style="display: none;"><br/>';
    ...
    echo'<b>Email: </b> ' . $new_user . ' 
												<input type="text" id="enew_useru" value="' . $new_user . '" style="display: none;"><br/>';
    echo'<b>Телефон: </b> ' . $phone . ' 
  ...
  echo '<p>
												...
												<a href="#" onclick="javascript:updateprof(\'' . $new_user . '\',\'' . $type . '\');">Сохранить профиль</a>
										      </p></fieldset>';
  echo'<fieldset>
  ...
  mysql_query("update cart set user_id = '" . $user_id . "' where user_id = '" . $session_id . "'");
  ...
  echo("<br/><b>Нет такого пользователя или невреный электронный адрес!!!</b><br/>");
  echo("Попробуйте еще раз вернувшись на предыдущую страницу!");
}
echo'</div></div></div>';
?>
